    ...
    fn pointer_frame(
        ...
        for event in events {
            ...
            let parent_surface = match event.surface.data::<SurfaceData>() {
                ...
                None => {
                    warn!(surface = ?event.surface.id(), kind = ?event.kind, "LUMO_PATCH: winit pointer event dropped - SurfaceData missing on event.surface");
                    eprintln!("[LUMO_PATCH][winit-pointer] event dropped (no SurfaceData): surface={:?} kind={:?}", event.surface.id(), event.kind);
                    continue;
                },
            };
            ...
            let mut window = match self.windows.get_mut().get_mut(&window_id) {
                ...
                None => {
                    let known: Vec<_> = self.windows.get_mut().keys().copied().collect();
                    warn!(?window_id, ?known, surface = ?event.surface.id(), parent = ?parent_surface.id(), kind = ?event.kind, "LUMO_PATCH: winit pointer event dropped - window_id not in registry (silent-continue bug)");
                    eprintln!("[LUMO_PATCH][winit-pointer] event dropped (unregistered window_id={:?}, known={:?}, surface={:?}, parent={:?}, kind={:?})", window_id, known, event.surface.id(), parent_surface.id(), event.kind);
                    continue;
                },
            };

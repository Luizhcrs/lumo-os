...
use smithay_client_toolkit::{
    ...
    output::OutputState, registry::RegistryState,
    shell::wlr_layer::{Anchor, KeyboardInteractivity, Layer, LayerShell, LayerSurface, LayerSurfaceConfigure,},
...
#[derive(Debug, Clone)]
struct OsdRequest { text: String, icon: OsdIcon, }
struct LumoOsd {
    registry: RegistryState, output_state: OutputState, shm: Shm,
    layer: LayerSurface, pool: SlotPool, width: u32, height: u32,
    running: bool, first_configured: bool,
    ipc_stream: Option<UnixStream>, ipc_rx_buf: Vec<u8>,
    current: Option<OsdRequest>, phase_elapsed: f32,
    alpha_anim: LAAnimator<f32>, visible: bool, last_frame: Instant,
}impl LumoOsd {
    fn redraw(&mut self, qh: &QueueHandle<LumoOsd>) {
        let (buffer, canvas) = match self.pool.create_buffer(
            self.width as i32, self.height as i32, self.width as i32 * 4, wl_shm::Format::Argb8888)
        {
            Ok(x) => x,
            Err(e) => { eprintln!("[lumo-osd] create_buffer err: {e}"); return; }
        };
        let mut pixmap = tiny_skia::PixmapMut::from_bytes(canvas, self.width, self.height)
            .expect("PixmapMut");
        ...
        if let Some(req) = self.current.clone() {
            ...
            if alpha > 0 { draw_osd_frame(&mut pixmap, &req, alpha); }
        }
    ...
    fn show_osd(&mut self, text: String, icon: OsdIcon) {
        ...
        self.alpha_anim = LAAnimator::new(ca, 1.0,
            AnimCurve::Bezier { curve: LACurve::ease_out_cubic(), duration: FADE_IN_DUR });
        self.visible = true; self.last_frame = Instant::now();
    }
    fn tick(&mut self, dt: f32, qh: &QueueHandle<LumoOsd>) {
        ...
        if self.phase_elapsed <= FADE_IN_DUR {
            ...
            if !self.alpha_anim.is_done() { let _ = self.alpha_anim.tick(dt); }
        } else {
            let lt = self.phase_elapsed - (FADE_IN_DUR + HOLD_DUR);
            if lt <= dt * 2.0 {
                self.alpha_anim = LAAnimator::new(1.0, 0.0,
                    AnimCurve::Bezier { curve: LACurve::ease_out_cubic(), duration: FADE_OUT_DUR });
            }
            let _ = self.alpha_anim.tick(dt);
        }
        if self.phase_elapsed > TOTAL_DUR {
            self.current = None; self.visible = false; self.redraw(qh); return;
        }
        self.redraw(qh);
    }
}fn draw_osd_frame(pixmap: &mut tiny_skia::PixmapMut, req: &OsdRequest, alpha: u8) {
    let w = pixmap.width() as f32; let h = pixmap.height() as f32;
    let bga = ((OSD_BG_ALPHA as u32 * alpha as u32) / 255) as u8;
    fill_rrect(pixmap, 0.0, 0.0, w, h, OSD_RADIUS, rgba_hex(OSD_BG_HEX, bga));
    let ic = rgba_hex(OSD_TEXT_HEX, alpha); let cy = h / 2.0;
    match req.icon {
    ...
    draw_text(pixmap, OSD_ICON_X+OSD_ICON_SIZE+10.0, (h-OSD_TEXT_SIZE*1.2)/2.0,
        &req.text, OSD_TEXT_SIZE, rgba_hex(OSD_TEXT_HEX, alpha), false);
}
fn draw_keyboard_icon(c: &mut tiny_skia::PixmapMut, cx: f32, cy: f32, sz: f32, col: tiny_skia::Color) {
    let y=cy-sz*0.4; let w=sz; let h=sz*0.65;
    stroke_rrect(c, cx, y, w, h, 2.5, col, 1.5);
    let kw=sz*0.22; let kh=sz*0.18; let kr=1.5; let ry=y+h*0.2;
    for i in 0..3i32 { fill_rrect(c, cx+sz*0.08+i as f32*(kw+sz*0.09), ry, kw, kh, kr, col); }
    let sw=sz*0.55;
    fill_rrect(c, cx+(w-sw)/2.0, ry+kh+sz*0.1, sw, kh*0.85, kr, col);
}
fn draw_volume_icon(c: &mut tiny_skia::PixmapMut, cx: f32, cy: f32, sz: f32, col: tiny_skia::Color) {
    let hh=sz*0.35;
    fill_rrect(c, cx, cy-hh, sz*0.4, hh*2.0, 1.0, col);
    for i in 1..=2u32 { let r=sz*0.25*i as f32; stroke_rrect(c,cx+sz*0.35,cy-r,r*0.4,r*2.0,r*0.2,col,1.5); }
}
fn draw_brightness_icon(c: &mut tiny_skia::PixmapMut, cx: f32, cy: f32, sz: f32, col: tiny_skia::Color) {
    use crate::bar::icons::fill_circle;
    fill_circle(c, cx+sz/2.0, cy, sz*0.22, col);
    let rl=sz*0.18; let rw=sz*0.07; let d=sz*0.32;
    for (dx,dy) in [(0.0f32,-d),(0.0,d),(-d,0.0),(d,0.0)] {
        fill_rrect(c, cx+sz/2.0+dx-rw/2.0, cy+dy-rl/2.0, rw, rl, 1.0, col);
    }
}
fn connect_ipc_osd() -> Option<UnixStream> {
    let path = default_socket_path()?;
    match UnixStream::connect(&path) {
        Ok(s) => { s.set_nonblocking(true).ok()?;
            eprintln!("[lumo-osd] IPC conectado em {}", path.display()); Some(s) }
        Err(e) => { eprintln!("[lumo-osd] IPC nao conectou ({})", e); None }
    }
}
fn drain_ipc_osd(s: &mut UnixStream, buf: &mut Vec<u8>) -> (bool, Option<(String, OsdIcon)>) {
    let mut tmp=[0u8;256]; let mut alive=true; let mut pending=None;
    ...
    while let Some(nl) = buf.iter().position(|b| *b == b'\n') {
        ...
        if let Ok(s2) = std::str::from_utf8(&line[..len.saturating_sub(1)]) {
            if let Ok(ev) = serde_json::from_str::<LumoEvent>(s2.trim()) {
                if let LumoEvent::ShowOsd { text, icon, .. } = ev { pending=Some((text,icon)); }
            }
        }
    }
    (alive, pending)
...
impl CompositorHandler for LumoOsd {
    fn scale_factor_changed(&mut self,_c:&Connection,_q:&QueueHandle<Self>,_s:&wl_surface::WlSurface,_f:i32){}
    fn transform_changed(&mut self,_c:&Connection,_q:&QueueHandle<Self>,_s:&wl_surface::WlSurface,_t:wl_output::Transform){}
    fn frame(&mut self,_c:&Connection,_q:&QueueHandle<Self>,_s:&wl_surface::WlSurface,_t:u32){}
...
impl ShmHandler for LumoOsd { fn shm_state(&mut self)->&mut Shm{&mut self.shm} }
impl LayerShellHandler for LumoOsd {
    fn closed(&mut self,_c:&Connection,_q:&QueueHandle<Self>,_l:&LayerSurface){self.running=false;}
    fn configure(&mut self,_c:&Connection,qh:&QueueHandle<Self>,_l:&LayerSurface,cfg:LayerSurfaceConfigure,_s:u32){
        self.width=if cfg.new_size.0==0{OSD_W}else{cfg.new_size.0};
        self.height=if cfg.new_size.1==0{OSD_H}else{cfg.new_size.1};
        self.first_configured=true; self.redraw(qh);
    }
}
...
delegate_registry!(LumoOsd);/// Entry point do binario `lumo-osd`.
pub fn run() {
    let _ = font_system(); let _ = swash_cache();
    ...
    let layer = layer_shell.create_layer_surface(&qh, surface, Layer::Overlay, Some("lumo-osd"), None);
    ...
    let ai = LAAnimator::new(0.0f32, 0.0f32,
        AnimCurve::Bezier { curve: LACurve::ease_out_cubic(), duration: FADE_IN_DUR });
    let mut state = LumoOsd {
        registry: RegistryState::new(&globals), output_state: OutputState::new(&globals, &qh),
        shm, layer, pool, width: OSD_W, height: OSD_H,
        running: true, first_configured: false,
        ipc_stream: connect_ipc_osd(), ipc_rx_buf: Vec::with_capacity(256),
        current: None, phase_elapsed: 0.0, alpha_anim: ai,
    ...
    while state.running {
        ...
        if state.current.is_some() { state.tick(dt, &qh); }
        conn.flush().ok();
        if let Some(guard) = queue.prepare_read() {
            use std::os::fd::AsFd; let fd = conn.as_fd();
            let tms: i32 = if state.current.is_some() { 16 } else { 50 };
            let mut pfd=[nix::poll::PollFd::new(fd,nix::poll::PollFlags::POLLIN)];
            let _ = nix::poll::poll(&mut pfd, nix::poll::PollTimeout::try_from(tms).expect("tms"));
            let _ = guard.read();
        }
        if let Err(e) = queue.dispatch_pending(&mut state) {
            let m=format!("{e:?}");
            if m.contains("ConnectionReset")||m.contains("BrokenPipe")||m.contains("InvalidObject") {
                eprintln!("[lumo-osd] compositor desconectou, saindo"); break;
            }
        }
        ...
        if last_ipc.elapsed() >= Duration::from_millis(16) {
            ...
            if let Some(mut s) = state.ipc_stream.take() {
                ...
                if alive { state.ipc_stream=Some(s); } else { eprintln!("[lumo-osd] IPC peer fechou"); }
                if let Some((text, icon)) = pending { state.show_osd(text, icon); }
            }
        }
    }
}
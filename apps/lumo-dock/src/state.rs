...
impl LumoDock {
    pub fn new(globals: GlobalList, qh: QueueHandle<Self>, shm: Shm, pool: SlotPool, layer: LayerSurface, cfg: DockConfig) -> Self {
        let n = cfg.slots.len();
        let mut scales = Vec::with_capacity(n + 1);
        for _ in 0..=n { let mut s = Spring::snappy(); s.snap_to(1.0); scales.push(s); }
        Self {
            registry: RegistryState::new(&globals),
            output_state: OutputState::new(&globals, &qh),
            shm, seat_state: SeatState::new(&globals, &qh),
            layer, pool, cfg, running: true, configured: false,
            width: crate::DOCK_W, scales, hover_idx: -1,
            running_procs: HashMap::new(), pointer: None,
            pointer_x: 0.0, pointer_y: 0.0, slot_rects: Vec::new(), trash_rect: None,
        }
    }
    pub fn animating(&self) -> bool { self.scales.iter().any(|s| !s.settled()) }
    pub fn tick_springs(&mut self, dt: f32) { for s in &mut self.scales { s.tick(dt); } }
    pub fn refresh_running(&mut self) {
        for slot in &self.cfg.slots {
            if slot.process.is_empty() { continue; }
            self.running_procs.insert(slot.process.clone(), is_process_running(&slot.process));
        }
    }
    pub fn redraw(&mut self, qh: &QueueHandle<Self>) {
        ...
        let (buffer, canvas) = match self.pool.create_buffer(w as i32, h as i32, (w*4) as i32, wl_shm::Format::Argb8888) {
            Ok(v) => v, Err(e) => { eprintln!("[lumo-dock] buf: {:?}", e); return; }
        };
        let mut pm = tiny_skia::PixmapMut::from_bytes(canvas, w as u32, h as u32).expect("pixmap");
        let (sr, tr) = paint::paint_dock(&mut pm, w as u32, h as u32, &self.cfg.slots, &self.scales, self.hover_idx, &self.running_procs);
        self.slot_rects = sr; self.trash_rect = tr;
...
fn is_process_running(name: &str) -> bool {
    let Ok(e) = std::fs::read_dir("/proc") else { return false; };
    for entry in e.flatten() {
        let f = entry.file_name(); let s = f.to_string_lossy();
        if s.chars().all(|c| c.is_ascii_digit()) {
            if std::fs::read_to_string(entry.path().join("comm")).unwrap_or_default().trim() == name {
                return true;
            }
        }
    }
...
impl ShmHandler for LumoDock { fn shm_state(&mut self) -> &mut Shm { &mut self.shm } }
impl LayerShellHandler for LumoDock {
    ...
    fn configure(&mut self, _: &Connection, qh: &QueueHandle<Self>, _: &LayerSurface, c: LayerSurfaceConfigure, _: u32) {
        ...
        self.configured = true; self.refresh_running(); self.redraw(qh);
    }
}
    ...
    fn new_capability(&mut self, _: &Connection, qh: &QueueHandle<Self>, seat: wl_seat::WlSeat, cap: Capability) {
        if cap == Capability::Pointer && self.pointer.is_none() {
            ...
            if let Ok(p) = self.seat_state.get_pointer_with_theme(
                qh, &seat, self.shm.wl_shm(), self.layer.wl_surface().clone(), ThemeSpec::System,
            ) { self.pointer = Some(p); }
        }
    }
...
impl PointerHandler for LumoDock {
    fn pointer_frame(&mut self, _: &Connection, qh: &QueueHandle<Self>,
        _: &smithay_client_toolkit::reexports::client::protocol::wl_pointer::WlPointer, events: &[PointerEvent]) {
        for ev in events {
            match ev.kind {
                PointerEventKind::Motion { .. } => {
                    self.pointer_x = ev.position.0 as f32;
                    let nh = crate::input::hit_test_slot(self.pointer_x, &self.slot_rects, self.trash_rect);
                    if nh != self.hover_idx {
                        if self.hover_idx >= 0 { let i = self.hover_idx as usize; if i < self.scales.len() { self.scales[i].set_target(1.0); } }
                        self.hover_idx = nh;
                        if nh >= 0 { let i = nh as usize; if i < self.scales.len() { self.scales[i].set_target(crate::MAGNIFY_MAX); } }
                    }
                    self.redraw(qh);
                }
                PointerEventKind::Leave { .. } => {
                    if self.hover_idx >= 0 { let i = self.hover_idx as usize; if i < self.scales.len() { self.scales[i].set_target(1.0); } }
                    self.hover_idx = -1; self.redraw(qh);
                }
                PointerEventKind::Press { button, .. } => {
                    if button == 0x110 { crate::input::handle_click(self.hover_idx, &self.cfg.slots); }
                }
                _ => {}
            }
        }

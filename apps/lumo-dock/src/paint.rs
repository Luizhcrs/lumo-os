    ...
    match name {
        ...
        "settings" => {
            ...
            for i in 0..4i32 {
                ...
                fill_rrect(
                    canvas,
                    cx + angle.cos() * q * 0.75 - tw * 0.5,
                    cy + angle.sin() * q * 0.75 - th * 0.5,
                    tw,
                    th,
                    1.0,
                    fg,
        ...
        "calendar" => {
            ...
            for row in 0..2i32 {
                for col in 0..3i32 {
                    fill_circle(
                        canvas,
                        cx - h * 0.55 + col as f32 * h * 0.42,
                        cy - h * 0.08 + row as f32 * h * 0.35,
                        2.5,
                        fg,
                    );
                }
...
pub fn paint_dock(
    ...
    for (i, slot) in slots.iter().enumerate() {
        ...
        draw_icon(
            ...
            ICON_SIZE * scale,
            &slot.icon,
            if hover_idx == i as i32 { pearl } else { muted },
        );

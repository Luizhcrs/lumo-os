...
fn authenticate_pam(password: &str) -> Result<(), String> {
    ...
    let mut client = Client::with_password("login")
        .map_err(|e| format!("PAM init: {e:?}"))?;
    client.conversation_mut().set_credentials(username.as_str(), password);
    client.authenticate()
...
fn authenticate_su(password: &str) -> Result<(), String> {
    ...
    let mut child = Command::new("su")
        ...
        .spawn().map_err(|e| format!("spawn su: {e}"))?;
    if let Some(mut stdin) = child.stdin.take() {
        let _ = stdin.write_all(password.as_bytes());
        let _ = stdin.write_all(b"\n");
    }
    let status = child.wait().map_err(|e| format!("wait: {e}"))?;
    if status.success() { Ok(()) } else { Err("Senha incorreta".to_string()) }
}

...
enum ItemKind { Submenu, Action, Separator }
...
fn build_menu() -> Vec<MenuItem> {
    vec![
        ...
        MenuItem { id: 10, label: "Nova nota",   parent_id: 1,  kind: ItemKind::Action    },
        MenuItem { id: 11, label: "Salvar",      parent_id: 1,  kind: ItemKind::Action    },
        MenuItem { id: 12, label: "",            parent_id: 1,  kind: ItemKind::Separator },
        MenuItem { id: 13, label: "Deletar",     parent_id: 1,  kind: ItemKind::Action    },
        MenuItem { id: 14, label: "",            parent_id: 1,  kind: ItemKind::Separator },
        MenuItem { id: 15, label: "Sair",        parent_id: 1,  kind: ItemKind::Action    },
        MenuItem { id: 2,  label: "Edit",        parent_id: 0,  kind: ItemKind::Submenu   },
        MenuItem { id: 20, label: "Buscar",      parent_id: 2,  kind: ItemKind::Action    },
...
impl LumoNotesMenu {
    ...
    fn layout_node(&self, id: i32, depth: i32) -> (i32, HashMap<String, OwnedValue>, Vec<OwnedValue>) {
        ...
        let children: Vec<OwnedValue> = if depth != 0 {
            ...
            self.items.iter().filter(|it| it.parent_id == id).map(|it| {
                ...
                let s = StructureBuilder::new().add_field(cid).add_field(cp).add_field(cc).build().unwrap();
    ...
    fn item_props(&self, id: i32) -> HashMap<String, OwnedValue> {
        ...
        if let Some(item) = self.items.iter().find(|it| it.id == id) {
            match item.kind {
                ItemKind::Separator => { p.insert("type".into(), OwnedValue::try_from(Value::from("separator")).unwrap()); }
                _ => { p.insert("label".into(), OwnedValue::try_from(Value::from(item.label)).unwrap()); }
            }
        }
        p
    }
...
impl LumoNotesMenu {
    ...
    fn get_layout(&self, pid: i32, depth: i32, _n: Vec<String>)
    ...
    fn get_group_properties(&self, ids: Vec<i32>, _n: Vec<String>)
...
fn serve_inner(tx: std::sync::mpsc::Sender<MenuAction>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let conn = Builder::session()?.serve_at(MENU_PATH, LumoNotesMenu::new(tx))?.build()?;
...
pub fn init_channel() -> std::sync::mpsc::Sender<MenuAction> {
    ...
    std::thread::Builder::new().name("notes-appmenu-bridge".into())
...
mod tests {
    ...
    #[test]
    fn test_menu_len() { assert_eq!(build_menu().len(), 12); }

    #[test]
    fn test_new_action() { assert!(matches!(id_to_action(10), Some(MenuAction::New))); }

    #[test]
    fn test_save_action() { assert!(matches!(id_to_action(11), Some(MenuAction::Save))); }

    #[test]
    fn test_delete_action() { assert!(matches!(id_to_action(13), Some(MenuAction::Delete))); }

    #[test]
    fn test_unknown_action() { assert!(id_to_action(999).is_none()); }
}

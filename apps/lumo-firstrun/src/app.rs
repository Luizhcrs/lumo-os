...
impl FirstRunApp {
    ...
    pub fn update(&mut self, msg: Msg) -> Task<Msg> {
        match msg {
            ...
            Msg::PasswordConfirmChanged(v) => {
                self.account.password_confirm = v;
                self.account.error = None;
                Task::none()
            }
            Msg::SelectWifi(ssid) => {
                self.selected_wifi = Some(ssid);
                self.wifi_password.clear();
                self.wifi_error = None;
                Task::none()
            }
            Msg::WifiPasswordChanged(v) => {
                self.wifi_password = v;
                Task::none()
            }
            Msg::ConnectWifi => {
                let ssid = match &self.selected_wifi {
                    Some(s) => s.clone(),
                    None => return Task::none(),
                };
                let pw = if self.wifi_password.is_empty() {
                    None
                } else {
                ...
                Task::perform(
                    async move {
                        tokio::task::spawn_blocking(move || {
                            system::connect_wifi(&ssid, pw.as_deref())
                        })
                        .await
                ...
                match result {
                    Ok(()) => {
                        if let Some(ref ssid) = self.selected_wifi {
                            if let Some(net) =
                                self.wifi_networks.iter_mut().find(|n| &n.ssid == ssid)
                            {
                                net.connected = true;
                            }
                        }
            ...
            Msg::Finish => {
                if let Err(e) = LocaleConfig::new(self.locale.code()).write() {
                    self.finish_error = Some(format!("locale: {e}"));
                    return Task::none();
                }
                ...
                if let Err(e) = system::mark_first_run_done(FIRST_RUN_FLAG) {
                    self.finish_error = Some(format!("flag: {e}"));
                    return Task::none();
                }
                std::process::exit(0);
            }
    ...
    pub fn view(&self) -> Element<Msg> {
        container(column![self.view_progress(), self.view_current()].spacing(0))
            .width(Length::Fill)
            .height(Length::Fill)
    ...
    fn view_progress(&self) -> Element<Msg> {
        ...
        container(row![
            container(Space::new(720.0 * ratio, 3.0)).style(|_| theme::progress_fill_style()),
    ...
    fn view_language(&self) -> Element<Msg> {
        ...
        for &loc in Locale::ALL {
            ...
            opts = opts.push(
                button(text(lbl).size(15))
                    .padding([10, 24])
                    .on_press(Msg::SelectLocale(loc)),
            );
        }
        container(
            column![
    ...
    fn view_account(&self) -> Element<Msg> {
        let err_elem: Element<Msg> = match &self.account.error {
            Some(e) => text(e.as_str())
                .size(13)
                .color(Color::from_rgb(0.97, 0.44, 0.44))
                .into(),
        ...
        container(
            column![
                ...
                text_input("Confirmar senha", &self.account.password_confirm)
                    .on_input(Msg::PasswordConfirmChanged)
                    .secure(true)
                    .padding(10)
                    .width(320.0),
    ...
    fn view_wifi(&self) -> Element<Msg> {
        ...
        let pw_elem: Element<Msg> = if self.selected_wifi.is_some() {
            text_input("Senha da rede", &self.wifi_password)
                .on_input(Msg::WifiPasswordChanged)
                .secure(true)
                .padding(10)
                .width(280.0)
                .into()
        ...
        let connect_elem: Element<Msg> = if self.selected_wifi.is_some() && !self.wifi_connecting {
            button(text("Conectar").size(14))
                .padding([10, 24])
                .on_press(Msg::ConnectWifi)
                .into()
    ...
    fn view_done(&self) -> Element<Msg> {
        ...
        container(
            column![
                ...
                text("Lumo OS instalado. Pronto pra usar.")
                    .size(24)
                    .color(Color::from_rgb(0.96, 0.96, 0.97)),
                Space::with_height(12.0),
                text("Clique em Iniciar para comecar.")
                    .size(15)
                    .color(Color::from_rgb(0.58, 0.59, 0.63)),
                err_elem,
                Space::with_height(48.0),
                button(text("Iniciar").size(16))
                    .padding([14, 40])
                    .on_press(Msg::Finish),
            ]
            .align_x(iced::alignment::Horizontal::Center)

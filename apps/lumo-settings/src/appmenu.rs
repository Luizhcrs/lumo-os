...
fn build_menu() -> Vec<MenuItem> {
    vec![
        ...
        MenuItem { id: 20, label: "Sobre lumo-settings", parent_id: 2,  kind: ItemKind::Action  },
    ]
}
            ...
            self.items.iter()
                ...
                .map(|it| {
                    ...
                    let s = StructureBuilder::new()
                        .add_field(cid).add_field(cprops).add_field(cchildren)
                        .build().unwrap();
...
impl LumoSettingsMenu {
    ...
    fn get_layout(&self, parent_id: i32, recursion_depth: i32, _names: Vec<String>)
    ...
    {
        if event_id == "clicked" {
            if let Some(a) = id_to_action(id) { let _ = self.tx.send(a); }
        }
        Ok(())
    }
...
fn serve_inner(tx: std::sync::mpsc::Sender<MenuAction>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let svc = conn.unique_name().map(|n| n.to_string()).unwrap_or_default();
    eprintln!("[appmenu] dbusmenu ativo em {} service={}", MENU_PATH, svc);
    {
        let reg = zbus::blocking::Proxy::new(
            &conn, "com.canonical.AppMenu.Registrar",
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            for action in std_rx {
                if let Some(tx) = MENU_TOK_TX.get() { let _ = tx.send(action); }
            }
        })
        .expect("spawn bridge");
    std_tx

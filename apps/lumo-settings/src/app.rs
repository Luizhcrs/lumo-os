...
use iced::widget::{
    button, column, container, row, scrollable, slider, text, text_input, Space, Svg,
};
...
impl App {
    ...
    pub fn update(&mut self, msg: Message) -> Task<Message> {
        match msg {
            ...
            Message::SetDayPreset => {
                self.brightness = 100;
                Task::perform(set_brightness(100), |_| Message::Nop)
            }
            Message::SetNightPreset => {
                self.brightness = 30;
                Task::perform(set_brightness(30), |_| Message::Nop)
            }
            ...
            Message::AccentSelected(a) => {
                self.accent = a;
                Task::none()
            }
            Message::ApplyAppearance => {
                self.status_msg = "Aparencia aplicada.".into();
                Task::perform(persist_appearance(self.dark_mode, self.accent), |_| {
                    Message::ApplyDone
                })
            }

            Message::KeyboardLayoutSelected(i) => {
                self.kbd_layout_idx = i;
                Task::none()
            }

            Message::TapToClickToggle => {
                self.tap_to_click = !self.tap_to_click;
                Task::none()
            }
            ...
            Message::ApplyTouchpad => {
                ...
                Task::perform(
                    persist_touchpad(self.tap_to_click, self.natural_scroll, self.accel_enabled),
                    |_| Message::ApplyDone,
                )
            }

            Message::ReducedMotionToggle => {
                self.a11y_reduced_motion = !self.a11y_reduced_motion;
                Task::none()
            }
            Message::HighContrastToggle => {
                self.a11y_high_contrast = !self.a11y_high_contrast;
                Task::none()
            }
            Message::FontScaleChanged(v) => {
                self.a11y_font_scale = v.clamp(0.8, 1.4);
                Task::none()
            }
            ...
            Message::ShowAbout => {
                self.status_msg = "lumo-settings 0.1.0".into();
                Task::none()
            }
            ...
            .map(|&t| {
                ...
                button(
                    row![
                        ...
                        Space::with_width(8),
                        text(t.label()).size(13).color(if active {
                            LumoTheme::accent()
                        } else {
                            LumoTheme::fg()
                        }),
    ...
    fn view_wifi(&self) -> Element<Message> {
        ...
        let networks: Vec<Element<Message>> = self
            .wifi_networks
            .iter()
            .enumerate()
            .map(|(i, net)| {
                ...
                row![
                    text(format!("{}{} ({}%)", net.ssid, conn_label, net.signal))
                        .size(13)
                        .color(LumoTheme::fg()),
    ...
    fn view_bluetooth(&self) -> Element<Message> {
        ...
        column![
            ...
            Space::with_height(16),
            text("Bluetooth: stub (nmcli bt nao disponivel)")
                .size(11)
                .color(LumoTheme::muted()),
        ]
        .spacing(0)
    ...
    fn view_audio(&self) -> Element<Message> {
        column![
            ...
            Space::with_height(8),
            slider(0..=100, self.volume, Message::VolumeChanged).width(Length::Fixed(320.0)),
            Space::with_height(4),
            text(format!("{}%", self.volume))
                .size(12)
                .color(LumoTheme::muted()),
            Space::with_height(12),
            text("Audio via pactl (stub)")
                .size(11)
                .color(LumoTheme::muted()),
        ]
        .spacing(0)
    ...
    fn view_battery(&self) -> Element<Message> {
        ...
        column![
            ...
            Space::with_height(16),
            text(format!("Nivel: {}%", self.battery_percent))
                .size(14)
                .color(LumoTheme::fg()),
            Space::with_height(4),
            text(format!("Saude: {}%", self.battery_health))
                .size(13)
                .color(LumoTheme::muted()),
            Space::with_height(16),
            button(text(limit_label).size(13).color(LumoTheme::fg()))
                ...
                .padding([8, 16]),
            Space::with_height(8),
            text("charge_control_end_threshold via samsung-galaxybook driver")
                .size(11)
                .color(LumoTheme::muted()),
        ]
        .spacing(0)
    ...
    fn view_appearance(&self) -> Element<Message> {
        ...
        let accents: Vec<Element<Message>> = AccentOption::ALL
            .iter()
            .map(|&a| {
    ...
    fn view_keyboard(&self) -> Element<Message> {
        ...
        column![
            ...
            Space::with_height(12),
            text("Layout stub (localectl set-keymap)")
                .size(11)
                .color(LumoTheme::muted()),
        ]
        .spacing(0)
    ...
    fn view_accessibility(&self) -> Element<Message> {
        let rm_label = if self.a11y_reduced_motion {
            ...
            "Reducir animacoes: Nao"
        };
        let hc_label = if self.a11y_high_contrast {
            "Alto contraste: Sim"
        } else {
            "Alto contraste: Nao"
        };
        column![
            ...
            button(text(rm_label).size(13).color(LumoTheme::fg()))
                ...
                .style(move |_, _| if self.a11y_reduced_motion {
                    ButtonStyle::Primary.style()
                } else {
                    ButtonStyle::Secondary.style()
                })
            ...
            button(text(hc_label).size(13).color(LumoTheme::fg()))
                ...
                .style(move |_, _| if self.a11y_high_contrast {
                    ButtonStyle::Primary.style()
                } else {
                    ButtonStyle::Secondary.style()
                })
            ...
            Space::with_height(16),
            text(format!(
                "Escala de fonte: {:.1}x (0.8 - 1.4)",
                self.a11y_font_scale
            ))
            .size(13)
            .color(LumoTheme::muted()),
            Space::with_height(8),
            slider(
                0..=100,
                ((self.a11y_font_scale - 0.8) / 0.006) as u8,
                move |v| { Message::FontScaleChanged(0.8 + v as f32 * 0.006) }
            )
            .width(Length::Fixed(320.0)),
            Space::with_height(16),
            button(text("Aplicar").size(13).color(LumoTheme::bg()))
...
async fn persist_appearance(dark: bool, accent: AccentOption) -> Result<(), String> {
    ...
    tokio::fs::write(format!("{}/appearance.toml", dir), content)
        .await
        .map_err(|e| e.to_string())?;
    Ok(())
}
...
async fn persist_touchpad(tap: bool, natural: bool, accel: bool) -> Result<(), String> {
    ...
    tokio::fs::write(format!("{}/touchpad.toml", dir), content)
        .await
        .map_err(|e| e.to_string())?;
    Ok(())
}

...
fn serve_inner(tx: std::sync::mpsc::Sender<MenuAction>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let conn = Builder::session()?
        .serve_at(MENU_PATH, LumoCalcMenu::new(tx))?
        .build()?;
...
mod tests {
    ...
    #[test]
    fn test_copy_action() {
        assert!(matches!(id_to_action(20), Some(MenuAction::CopyResult)));
    }

    #[test]
    fn test_about_action() {
        assert!(matches!(id_to_action(30), Some(MenuAction::ShowAbout)));
    }
}

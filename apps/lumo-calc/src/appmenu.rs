//! appmenu.rs -- DBus appmenu para lumo-calc.
...
fn build_menu() -> Vec<MenuItem> {
    vec![
        ...
        MenuItem { id: 20, label: "Copiar resultado", parent_id: 2,  kind: ItemKind::Action  },
...
const MENU_PATH: &str = "/com/lumo/lumo_calc/menus/main";
...
fn serve_inner(tx: std::sync::mpsc::Sender<MenuAction>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let conn = Builder::session()?.serve_at(MENU_PATH, LumoCalcMenu::new(tx))?.build()?;
...
pub fn init_channel() -> std::sync::mpsc::Sender<MenuAction> {
    ...
    std::thread::Builder::new().name("calc-appmenu-bridge".into())
...
mod tests {
    ...
    #[test]
    fn test_menu_len() { assert_eq!(build_menu().len(), 7); }

    #[test]
    fn test_quit_action() { assert!(matches!(id_to_action(10), Some(MenuAction::Quit))); }

    #[test]
    fn test_copy_action() { assert!(matches!(id_to_action(20), Some(MenuAction::CopyResult))); }

    #[test]
    fn test_about_action() { assert!(matches!(id_to_action(30), Some(MenuAction::ShowAbout))); }
}

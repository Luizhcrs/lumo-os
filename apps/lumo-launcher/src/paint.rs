...
pub fn paint_launcher(canvas: &mut PixmapMut, input: &PaintInput) -> Vec<(f32, f32)> {
    ...
    let sw = input.width as f32; let sh = input.height as f32;
    let px = (sw - PANEL_W) * 0.5; let py = sh * 0.32;
    ...
    for i in 1..=4u8 {
        let e = i as f32;
        fill_rrect(canvas, px - e, py + e * 2.0, PANEL_W + e * 2.0, panel_h, PANEL_RADIUS, rgba(0x000000, 80u8.saturating_sub(i * 15)));
    }
    fill_rrect(canvas, px, py, PANEL_W, panel_h, PANEL_RADIUS, rgba(0x1a1a21, 0xF5));

    let sx = px + 16.0; let sy = py + 12.0; let sw2 = PANEL_W - 32.0;
    fill_rrect(canvas, sx, sy, sw2, SEARCH_BOX_H, 10.0, rgba(0x0a0a0c, 0xCC));

    // Cursor
    let cursor_x = sx + 12.0 + input.query.len() as f32 * 8.5;
    fill_rect_c(canvas, cursor_x, sy + 10.0, 2.0, SEARCH_BOX_H - 20.0, rgba(0x10b981, 0xFF));

    // Placeholder dots
    if input.query.is_empty() {
        for i in 0..12u32 {
            fill_circle(canvas, sx + 14.0 + i as f32 * 8.0, sy + SEARCH_BOX_H * 0.5, 1.5, rgba(0x9596a0, 0x66));
        }
    }
    ...
    if let Some(math) = input.math_result {
        fill_rrect(canvas, px + 8.0, ry, PANEL_W - 16.0, ROW_H, 8.0, rgba(0x10b981, 0x22));
        fill_circle(canvas, px + 24.0, ry + ROW_H * 0.5, 8.0, accent);
        for (i, _) in math.chars().take(20).enumerate() {
            fill_circle(canvas, px + 44.0 + i as f32 * 9.0, ry + ROW_H * 0.5, 2.5, pearl);
        }
    ...
    for (i, entry) in input.results.iter().enumerate().take(MAX_RESULTS) {
        ...
        if sel { fill_rrect(canvas, px + 8.0, row_y, PANEL_W - 16.0, ROW_H, 8.0, rgba(0x10b981, 0x2A)); }
        let icon_bg = if sel { rgba(0x10b981, 0x55) } else { rgba(0x3f3f46, 0xFF) };
        fill_rrect(canvas, px + 14.0, row_y + ROW_H * 0.5 - 14.0, 28.0, 28.0, 7.0, icon_bg);
        ...
        for (j, _) in entry.name.chars().take(30).enumerate() {
            fill_circle(canvas, px + 52.0 + j as f32 * 7.5, row_y + ROW_H * 0.45, 2.0, fg);
        }

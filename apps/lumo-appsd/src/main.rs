...
use iced::{daemon, window, Size, Subscription, Task, Theme};
...
fn send_wm_app_deactivated() {
    std::thread::spawn(|| {
        ...
        let runtime = match std::env::var("XDG_RUNTIME_DIR") {
            Ok(r) => r,
            Err(_) => {
                eprintln!("[appsd] W34.11 sem XDG_RUNTIME_DIR");
                return;
            }
        };
        let path = format!("{}/lumo-wm.sock", runtime);
...
fn update(state: &mut State, msg: Msg) -> Task<Msg> {
    match msg {
        ...
        Msg::Opened(AppKind::Store, id) => {
            ...
            let arg = state
                .pending_args
                .get_mut(&AppKind::Store)
                .and_then(|q| q.pop_front());
            let (app, t) = lumo_store::app::StoreApp::new();
            if let Some(tab) = arg {
                eprintln!(
                    "[appsd] store tab={} (StoreApp::new sem tab arg ainda)",
                    tab
                );
            }

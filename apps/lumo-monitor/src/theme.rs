...
impl LumoTheme {
    ...
    pub fn warn() -> Color    { Color::from_rgba(0.85, 0.47, 0.08, 1.0) }
    pub fn danger() -> Color  { c(LFTokens::DANGER_SRGB) }
}

pub enum TabStyle { Active, Inactive }
    ...
    pub fn style(&self) -> button::Style {
        match self {
            TabStyle::Active => button::Style {
                ...
                border: Border { color: LumoTheme::accent(), width: 1.0, radius: 4.0.into() },
...
pub fn container_bg() -> container::Style {
    container::Style { background: Some(Background::Color(LumoTheme::bg())), ..Default::default() }
}
...
pub fn pct_color(pct: f32) -> Color {
    if pct < 60.0 { LumoTheme::accent() }
    else if pct < 85.0 { LumoTheme::warn() }
    else { LumoTheme::danger() }
}

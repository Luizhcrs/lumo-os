...
impl App {
    ...
    pub fn update(&mut self, msg: Message) -> Task<Message> {
        match msg {
            ...
            Message::ShowAbout => {
                self.status = "lumo-monitor 0.1.0".into();
                Task::none()
            }
        }
    }
    ...
    pub fn view(&self) -> Element<Message> {
        let tab_bar: Vec<Element<Message>> = Tab::ALL
            ...
            .map(|&t| {
                ...
                button(text(t.label()).size(12).color(if active {
                    LumoTheme::accent()
                } else {
                    LumoTheme::muted()
                }))
                ...
                .style(move |_, _| {
                    if active {
                        TabStyle::Active.style()
                    } else {
                        TabStyle::Inactive.style()
                    }
                })
                .padding([6, 14])
                .into()
        ...
        let status = container(text(self.status.clone()).size(11).color(LumoTheme::muted()))
            .padding([4, 12]);
    ...
    fn view_memory(&self) -> Element<Message> {
        ...
        container(
            column![
                ...
                text(format!(
                    "Disponivel: {:.2} GB",
                    self.mem.available_kb as f32 / 1_048_576.0
                ))
                .size(12)
                .color(LumoTheme::muted()),
                text(format!("Buffers: {} MB", self.mem.buffers_kb / 1024))
                    .size(12)
                    .color(LumoTheme::muted()),
                text(format!("Cache:   {} MB", self.mem.cached_kb / 1024))
                    .size(12)
                    .color(LumoTheme::muted()),
            ]
            .padding(16)
            ...
            .map(|iface| {
                ...
                container(
                    row![
                        ...
                        Space::with_width(Length::Fill),
                        column![
                            text(format!("RX: {} KB  ({:.1} KB/s)", rx_kb, rx_rate_kb))
                                .size(11)
                                .color(LumoTheme::muted()),
                            text(format!("TX: {} KB  ({:.1} KB/s)", tx_kb, tx_rate_kb))
                                .size(11)
                                .color(LumoTheme::muted()),
                        ]
                        .spacing(2),
    ...
    fn view_processes(&self) -> Element<Message> {
        ...
        let proc_rows: Vec<Element<Message>> = self
            .processes
            .iter()
            .take(30)
            .map(|p| {
                ...
                row![
                    text(p.pid.to_string())
                        ...
                        .color(LumoTheme::muted())
                        .width(Length::Fixed(60.0)),
                    text(truncate(&p.name, 18))
                        .size(11)
                        .color(LumoTheme::fg())
                        .width(Length::Fixed(150.0)),
                    text(format!("{:.1}", p.cpu_pct))
                        .size(11)
                        .color(cpu_color)
                        .width(Length::Fixed(60.0)),
                    text(format!("{} KB", p.rss_kb))
                        .size(11)
                        .color(LumoTheme::muted())
                        .width(Length::Fixed(80.0)),
                    text(truncate(&p.cmd, 40))
                        .size(11)
                        .color(LumoTheme::muted()),
                ]
                .align_y(Alignment::Center)
        ...
        container(
            column![
                ...
                Space::with_height(8),
                text(format!("{} processos", self.processes.len()))
                    .size(11)
                    .color(LumoTheme::muted()),
                Space::with_height(8),
                header,

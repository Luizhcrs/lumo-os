...
impl ClosedFormSpring {
    ...
    pub fn settled(&self, t_secs: f32, from: f32, to: f32, initial_velocity: f32, epsilon: f32) -> bool {
    ...
    pub fn omega_n(&self) -> f32 { self.omega_n }

    /// Razao de amortecimento zeta = c / (2 * sqrt(k*m)).
    pub fn zeta(&self) -> f32 { self.zeta }

    /// Regime de amortecimento calculado.
    pub fn regime(&self) -> DampingRegime { self.regime }
}
...
mod tests {
    ...
    fn critically_damped_sem_overshoot() {
        ...
        assert!(max_v <= 1.001, "critically-damped nao deve overshoot: max={max_v}");
    }
    ...
    fn overdamped_sem_overshoot_e_lento() {
        ...
        assert!(v_at_03 < 0.99, "overdamped deve ser lento em 0.3s: v={v_at_03}");
    }
    ...
    fn velocidade_em_t0_igual_initial_velocity() {
        ...
        for preset in presets {
            ...
            assert_near(vel, v_init, 0.01, &format!("velocity_at(0) preset={preset:?}"));
        }
    }
    ...
    fn preset_tap_feedback_regime_underdamped() {
        ...
        assert_eq!(s.regime(), DampingRegime::Underdamped);
        assert!((s.zeta() - 0.866).abs() < 0.01, "zeta TapFeedback: {}", s.zeta());
    }
    ...
    fn preset_window_open_close_regime_underdamped() {
        ...
        assert_eq!(s.regime(), DampingRegime::Underdamped);
        assert!((s.zeta() - 0.844).abs() < 0.01, "zeta WindowOpenClose: {}", s.zeta());
    }
    ...
    fn preset_sheet_slide_regime_near_critical() {
        ...
        assert!(
            s.zeta() > 0.95,
            "SheetSlide deve ter zeta alto (near-critical): {}", s.zeta()
        );
    }
    ...
    fn preset_drag_to_reveal_regime_critical_or_overdamped() {
        ...
        assert!(
            s.regime() == DampingRegime::Critical || s.regime() == DampingRegime::Overdamped,
            "DragToReveal deve ser critico: regime={:?}", s.regime()
        );

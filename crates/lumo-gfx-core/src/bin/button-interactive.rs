//! button-interactive: Layer 4.1.9 — primeiro widget Lumo reativo.
//!
//! 3 botoes empilhados verticalmente sobre fundo pearl, mesmos presets do
//! `button-demo`, mas agora reativos:
//!   - mouse hover -> bg brightness +10%
//!   - mouse press -> scale 0.96 + shadow reduzido (spring anim)
//!   - release dentro -> click logado, volta pra hover
//!   - drag out -> cancela press
//!
//! Loop:
//!   1. winit_event -> InputBridge.map -> LumoEvent
//!   2. ButtonHandle.handle_event (atualiza state machine)
//!   3. ButtonHandle.update(dt) (integra spring)
//!   4. Button::queue_stateful (renderer pega state + press_progress)
...
// ----------------------------------------------------------------------------
// Renderer dedicado ao demo (mantem 3 handles entre frames).
// ----------------------------------------------------------------------------
struct ButtonInteractive {
...
impl ButtonInteractive {
    async fn new(window: Arc<Window>) -> Self {
        ...
        let (device, queue) = adapter
            .request_device(
                &wgpu::DeviceDescriptor {
                    label: Some("button-interactive::device"),
        ...
        let buttons = [
            Button::primary().with_label("Pressione"),
            Button::ghost().with_label("Cancelar"),
            Button::danger().with_label("Apagar"),
        ];
        let handles = [
            ButtonHandle::new(),
            ButtonHandle::new(),
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        let measured: Vec<[f32; 2]> = self
            .buttons
            .iter()
            .map(|b| b.measure(&mut self.text))
            .collect();
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("button-interactive::encoder"),
            });

        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("button-interactive::quads-pass"),
...
struct App {
    ...
    renderer: Option<ButtonInteractive>,
}
